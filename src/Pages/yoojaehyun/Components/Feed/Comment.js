...
class Comment extends Component {
  isLikedToggle = () => {
    this.props.isLikedHandle(this.props.commentList);
  };
  handleDelete = () => {
    this.props.onHandleDelete(this.props.commentList);
  };
  render() {
    const { userName, comment, isLiked } = this.props.commentList;
    return (
      <li>
        <div className='row'>
        ...
        <div className='like__delete'>
          <button className='likeButton' onClick={this.isLikedToggle}>
            <svg
              aria-label='좋아요 취소'
              fill=''
              height='12'
              ...
              width='12'>
              <path
                className={`heart  ${isLiked ? 'heart__fill' : ''}`}
